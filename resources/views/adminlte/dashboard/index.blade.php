...
@section('content')
    ...
    <div class="row">
        <div class="col-lg-3 col-6">
            <x-adminlte-small-box title="{{ $stats['users'] }}" text="{{ __('adminlte.users') }}" theme="primary"
                icon="bi bi-people" :url="\Illuminate\Support\Facades\Route::has('adminlte.users.index')
                    ? route('adminlte.users.index')
                    : null" />
        </div>
        <div class="col-lg-3 col-6">
            <x-adminlte-small-box title="{{ $stats['projects'] }}" text="{{ __('adminlte.projects') }}" theme="success"
                icon="bi bi-kanban" :url="\Illuminate\Support\Facades\Route::has('adminlte.projects.index')
                    ? route('adminlte.projects.index')
                    : null" />
        </div>
        <div class="col-lg-3 col-6">
            <x-adminlte-small-box title="{{ $stats['unread_messages'] }}" text="{{ __('adminlte.messages') }}"
                theme="warning" icon="bi bi-envelope" :url="\Illuminate\Support\Facades\Route::has('adminlte.mailbox.index')
                    ? route('adminlte.mailbox.index')
                    : null" />
        </div>
        <div class="col-lg-3 col-6">
            <x-adminlte-small-box title="{{ $stats['events'] }}" text="{{ __('adminlte.events') }}" theme="info"
                icon="bi bi-calendar-event" :url="\Illuminate\Support\Facades\Route::has('adminlte.calendar.index')
                    ? route('adminlte.calendar.index')
                    : null" />
        </div>
    </div>
                ...
                <div class="list-group list-group-flush">
                    @forelse ($recentActivity as $entry)
                        <div class="list-group-item">
                            <div class="d-flex justify-content-between">
                                <span><span
                                        class="badge text-bg-secondary me-2">{{ $entry->event }}</span>{{ $entry->description }}</span>
                                <small
                                    class="text-secondary">{{ \Illuminate\Support\Carbon::parse($entry->created_at)->diffForHumans() }}</small>
                            </div>
